...
export const gameGuestTypeEnum = pgEnum("game_guest_type", [
  "regular",
  "one_time",
]);
...
export const insertFamilyMemberSchema = createInsertSchema(
  familyMembersTable,
...
export const choresTable = pgTable("chores", {
  ...
  title: text("title").notNull(),
  description: text("description"),
  assignedTo: integer("assigned_to").references(() => familyMembersTable.id, {
    onDelete: "set null",
  }),
  ...
  approvedByParentId: integer("approved_by_parent_id").references(
    () => familyMembersTable.id,
    { onDelete: "set null" },
  ),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});

export const insertChoreSchema = createInsertSchema(choresTable).omit({
  id: true,
  createdAt: true,
});
...
export const choreTemplatesTable = pgTable("chore_templates", {
  ...
  createdBy: integer("created_by").references(() => familyMembersTable.id, {
    onDelete: "set null",
  }),
...
export const insertChoreTemplateSchema = createInsertSchema(
  choreTemplatesTable,
).omit({ id: true, createdAt: true });
...
export const choreTemplateChildrenTable = pgTable("chore_template_children", {
  ...
  templateId: integer("template_id")
    .notNull()
    .references(() => choreTemplatesTable.id, { onDelete: "cascade" }),
...
export const choreInstancesTable = pgTable(
  "chore_instances",
  {
    id: serial("id").primaryKey(),
    templateId: integer("template_id").references(
      () => choreTemplatesTable.id,
      { onDelete: "cascade" },
    ),
    childId: integer("child_id").references(() => familyMembersTable.id, {
      onDelete: "set null",
    }),
    title: text("title").notNull(),
    pointsValue: integer("points_value").notNull().default(10),
  ...
  (t) => [
    unique("unique_chore_instance_per_day").on(
      t.templateId,
      t.childId,
      t.dueDate,
    ),
...
export const insertChoreInstanceSchema = createInsertSchema(
  choreInstancesTable,
).omit({ id: true, createdAt: true });
...
export const redemptionsTable = pgTable("reward_redemptions", {
  ...
  approvedAt: timestamp("approved_at"),
  fulfilledByParentId: integer("fulfilled_by_parent_id").references(
    () => familyMembersTable.id,
    { onDelete: "set null" },
  ),
...
export const pointTransactionsTable = pgTable("point_transactions", {
  ...
  choreInstanceId: integer("chore_instance_id").references(
    () => choreInstancesTable.id,
    { onDelete: "set null" },
  ),
  redemptionId: integer("redemption_id").references(() => redemptionsTable.id, {
    onDelete: "set null",
  }),
  approvedByParentId: integer("approved_by_parent_id").references(
    () => familyMembersTable.id,
...
export const insertStreakMilestoneSchema = createInsertSchema(
...
export const insertPointMilestoneSchema = createInsertSchema(
...
export const insertChoreMilestoneSchema = createInsertSchema(
...
export const insertMealPlanSchema = createInsertSchema(mealPlanTable).omit({
  id: true,
});
...
export const insertRoutineItemSchema = createInsertSchema(
  routineItemsTable,
).omit({ id: true });
...
export const insertRoutineCompletionSchema = createInsertSchema(
  routineCompletionsTable,
).omit({ id: true, completedAt: true });
export type InsertRoutineCompletion = z.infer<
  typeof insertRoutineCompletionSchema
>;
...
export const gamesTable = pgTable("games", {
  ...
  resultType: gameResultTypeEnum("result_type").notNull().default("winner"),
...
export const gameGuestsTable = pgTable("game_guests", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  nickname: text("nickname"),
  avatarUrl: text("avatar_url"),
  avatarEmoji: text("avatar_emoji"),
  guestType: gameGuestTypeEnum("guest_type").notNull().default("regular"),
  active: boolean("active").notNull().default(true),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
...
export const gameParticipantsTable = pgTable("game_participants", {
  ...
  guestId: integer("guest_id").references(() => gameGuestsTable.id, {
    onDelete: "set null",
  }),
...
export const insertGameGuestSchema = createInsertSchema(gameGuestsTable).omit({
  id: true,
  createdAt: true,
});
export type Game = typeof gamesTable.$inferSelect;
export type GameGuest = typeof gameGuestsTable.$inferSelect;
...
export const settingsTable = pgTable("settings", {
  ...
  screensaverPhotoInterval: integer("screensaver_photo_interval")
    .notNull()
    .default(15),
});
...
export const insertScreensaverPhotoSchema = createInsertSchema(
  screensaverPhotosTable,
).omit({ id: true, createdAt: true });
export type InsertScreensaverPhoto = z.infer<
  typeof insertScreensaverPhotoSchema
>;
